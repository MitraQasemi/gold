...
const func = (app) => {
    ...
    app.use(route);
    route.post("/admin/config", validate(configValidation.create), isAuth, attachCurrentAdmin, isCan("create", "Config"), async (req, res, next) => {
        try {
    ...
    route.put("/admin/config/:id", validate(configValidation.update), isAuth, attachCurrentAdmin, isCan("update", "Config"), async (req, res, next) => {
        try {
